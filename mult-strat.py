...
# Reward/Risk ratios per strategy
REWARD_RISK = {
    "EMA_MACD": 2.0,
    "RSI": 1.5,
    "Stochastic": 1.5,
...
TRAIL_MULT = 0.5
CORR_THRESHOLD = 0.8
open_trades = {}  # key: pair, value: {units, stop_loss, take_profit, strategy}

# Adaptive strategy performance tracking
...
def add_indicators(df):
    ...
    df['pivot']=(df['high'].rolling(PIVOT_PERIOD).max()+df['low'].rolling(PIVOT_PERIOD).min()+df['close'].rolling(PIVOT_PERIOD).mean())/3
...
def strat_stoch(df):
    ...
    elif last['stoch_k']>80 and last['stoch_k']<df['stoch_d'].iloc[-2]: signal=-1
    return 0,signal
...
def strat_vwap(df):
    last=df.iloc[-1]; signal=0
    if last['close']<last['vwap'] and last['close']>df['vwap'].iloc[-2]: signal=1
    elif last['close']>last['vwap'] and last['close']<df['vwap'].iloc[-2]: signal=-1
    return 0,signal
...
def update_strategy_perf(strategy, profit):
    ...
    strategy_perf[strategy].append(profit)
...
if __name__=="__main__":
    while True:
        ...
        # Fetch all M5 prices for correlation
        prices = pd.DataFrame({pair: fetch_candles(pair)['close'] for pair in INSTRUMENTS})
        cor_matrix = prices.pct_change().rolling(50).corr().iloc[-len(INSTRUMENTS):]
        ...
        for pair in INSTRUMENTS:
            ...
            # Weighted multi-strategy scoring with adaptive weights
            ...
            def can_trade(pair, open_trades, threshold=CORR_THRESHOLD):
            ...
            # BUY
            if total_score >= 1.0 and h4_trend>0 and can_trade(pair, open_trades):
                ...
                open_trades[pair]={"units":units,"stop_loss":stop_loss,"take_profit":take_profit,"strategy":strategy}
                log_trade(pair,units,"Weighted Buy",total_score,last_price)

            # SELL
            elif total_score <= -1.0 and h4_trend<0 and can_trade(pair, open_trades):
                strategy=max(signals,key=lambda k: abs(signals[k]*STRATEGY_WEIGHTS[k]))
                rr = REWARD_RISK[strategy]
                units=-int((equity*RISK_PER_TRADE)/atr)
                stop_loss=last_price+atr
            ...
            # Update trailing stops and strategy performance
            for trade_pair,trade in list(open_trades.items()):
                current_price = df_m5['close'].iloc[-1]
                if trade['units']>0:
                    new_sl = max(trade['stop_loss'], current_price - TRAIL_MULT*atr)
                    profit = current_price - trade['stop_loss']
                else:
                    new_sl = min(trade['stop_loss'], current_price + TRAIL_MULT*atr)
                    profit = trade['stop_loss'] - current_price
                trade['stop_loss'] = new_sl
                update_strategy_perf(trade['strategy'], profit/atr)  # normalize by ATR
