...
def place_order(pair, units, stop_loss=None, take_profit=None, strategy="Bot"):
    ...
    if 'orderFillTransaction' in order_data:
        trade_id = order_data['orderFillTransaction']['tradeOpened']['tradeID']
        print(f"{datetime.now()} | Order executed: {pair} {units} units")
        return trade_id
    else:
        print(f"{datetime.now()} | Order placed but not filled: {pair}")
        return None
    ...
    while True:
        ...
        for pair in INSTRUMENTS:
            ...
            if total_score >= 1.0 and h4_trend > 0 and can_trade(pair):
                ...
                trade_id = place_order(pair, units, stop_loss, take_profit, strategy)
                if trade_id:
                    open_trades[pair] = {"id": trade_id, "units": units,
                                         "stop_loss": stop_loss, "take_profit": take_profit,
                                         "strategy": strategy}
